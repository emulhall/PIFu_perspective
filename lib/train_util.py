...
def gen_mesh(opt, net, cuda, data, save_path, use_octree=True, calc_chamfer=False):
    image_tensor = data['img'].to(device=cuda)
    calib_tensor = data['calib'].to(device=cuda)
    transforms_tensor = data['transforms'].to(device=cuda)
    ...
    try:
        ...
        verts, faces, _, _ = reconstruction(
            net, cuda, calib_tensor, opt.resolution, b_min, b_max, use_octree=use_octree, transform=transforms_tensor, num_samples=5000)
        ...
        if calc_chamfer:
            ...
            chamfer = chamferdist(verts, mesh, bidirectional=True)
            return chamfer
        else:
            return None
...
def gen_mesh_color(opt, netG, netC, cuda, data, save_path, use_octree=True):
    ...
    try:
        ...
        verts, faces, _, _ = reconstruction(
            netG, cuda, calib_tensor, opt.resolution, b_min, b_max, use_octree=use_octree, transform=transforms_tensor)
        ...
        for i in range(len(color) // interval):
            ...
            if i == len(color) // interval - 1:
                right = -1
            netC.query(verts_tensor[:, :, left:right], calib_tensor, transforms=transforms_tensor)
...
def calc_error(opt, net, cuda, dataset, num_tests):
    ...
    with torch.no_grad():
        ...
        for idx in tqdm(range(num_tests)):
            ...
            res, error = net.forward(image_tensor, sample_tensor, calib_tensor, labels=label_tensor, transforms=transforms_tensor)
